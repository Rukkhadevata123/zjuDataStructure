...
class CuckooHashTable {
    static constexpr int INITIAL_TABLE_SIZE = 23;
    static constexpr double MAX_LOAD_FACTOR = 0.5;
    static constexpr int MAX_REHASH_ATTEMPTS = 50;
    ...
    // 计算负载因子
    [[nodiscard]] auto loadFactor() const -> double {
...
public:
    ...
    // 查找元素
    [[nodiscard]] auto find(const Key &key) const -> bool {
        for (int i = 0; i < tables.size(); ++i) {
            int index = hash_functions.hash(key, i, table_size);
